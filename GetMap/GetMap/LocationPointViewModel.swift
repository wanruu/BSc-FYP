...
struct BuildingPoint: View {
    ...
    var body: some View {
        let x = centerX + CGFloat((building.longitude - locationGetter.current.coordinate.longitude)*85390*2) * scale + offset.x
        let y = centerY + CGFloat((locationGetter.current.coordinate.latitude - building.latitude)*111000*2) * scale + offset.y
