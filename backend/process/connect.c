...
double coor_t_dist (coor_t p1, coor_t p2) {
    double diff_x = (p1.lat - p2.lat) * laScale;
    double diff_y = (p1.lng - p2.lng) * lgScale;
    double diff_z = p1.alt - p2.alt;
    ...
    for (int i = 0; i < trajs_size; i++) { // for each traj
        ...
        for (int j = i + 1; j < trajs_size; j++) { // for next traj
            ...
            for (int k = 0; k < trajs[j].points_num; k++) { // for each point in next traj
                ...
                if(coor_t_dist (start, point) <= MinDist) {
                ...
                if(coor_t_dist (end, point) <= MinDist) {
