...
void compute_distance (coor_t locs[], double* dists) {
    ...
    double length1 = dist(points[0], points[1]); // length of p1p2
    double length2 = dist(points[2], points[3]); // length of p3p4
    ...
    double l_perp1 = dist(proj_p3, points[2]);
    double l_perp2 = dist(proj_p4, points[3]);
    double perp = (l_perp1 * l_perp1 + l_perp2 * l_perp2) / (l_perp1 + l_perp2);
    // parallel distance
    double l_para1 = min(dist(proj_p3, points[0]), dist(proj_p3, points[1]));
    double l_para2 = min(dist(proj_p4, points[0]), dist(proj_p4, points[1]));
...
double weighted_distance (coor_t locs[]) {
    ...
    return 0.8 * dists[0] + dists[1] + dists[2];
}